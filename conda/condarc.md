...
列出所有源的位置，conda 按照列出的顺序检索源。

使用 `defaults` 自动包含所有默认源。没有 URL 的表示 Anaconda.org 用户名。

### always_yes

在被要求继续时总是选择 `yes`，例如安装包时。和命令行中使用 `--yes` flag 一样。默认 `False`。示例：
...
在下载和使用 `conda list` 查看时显示源的 URL。默认 `False`。例如：
...
在使用 `-c` 或 `--channel` flag 为 conda 指定源时，如果指定的不是 URL，则 conda 会自动在前面添加 `channel_alias`。`channel_alias` 默认为 https://conda.anaconda.org 。

如果将 `channel_alias` 设置为 https://my.anaconda.repo:8080/conda/ ，当用户运行命令 `conda install -c conda-forge some-package`，conda 会从 https://my.anaconda.repo:8080/conda/conda-forge 查找 `some-package`。
...
可以将 `channel_alias` 设置为自己的源。

例如，将 `channel_alias` 设置为 https://your.repo.com ：

```sh
channel_alias: https://your.repo/
```

在 Windows 系统，在 URL 默认必须添加 `/`，如 https://your.repo/conda/ 。
